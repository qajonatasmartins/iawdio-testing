 ...
 */
export async function iawdio(prompt: string, { timeoutMsg, mapeamento }: { timeoutMsg?: string, mapeamento?: string }): Promise<void> {
    const xml = await getPageXML()
    const comando = await parseAndGenerateCommand(prompt, xml, timeoutMsg, mapeamento)
